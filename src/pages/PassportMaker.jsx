...
import {
  Upload, Crop, ImageSquare, TShirt, GridFour, Receipt, Printer, ArrowLeft, ArrowRight,
  Plus, Minus, Trash, Copy, Check, DownloadSimple, IdentificationCard,
...
const STEPS = [
  ...
  { id: 5, name: "Dress", icon: TShirt },
  { id: 6, name: "Layout", icon: GridFour },
  { id: 7, name: "Billing", icon: Receipt },
  { id: 8, name: "Print", icon: Printer },
];
...
export default function PassportMaker() {
  ...
  const goto = (s) => {
    ...
    setStep(Math.max(1, Math.min(8, s)));
  };
  ...
  return (
    <div>
                ...
                <button
                  ...
                  className={`h-9 pl-2 pr-3 rounded-full flex items-center gap-2 text-xs uppercase tracking-[0.15em] border transition
                    ${active ? "bg-zinc-900 text-white border-zinc-900" :
                      done ? "bg-[#0052FF]/10 text-[#0052FF] border-[#0052FF]/30" :
                      "bg-white text-zinc-500 border-zinc-200 hover:border-zinc-300"}`}
                >
                  <span className={`w-5 h-5 rounded-full flex items-center justify-center ${active ? "bg-white text-zinc-900" : done ? "bg-[#0052FF] text-white" : "bg-zinc-100 text-zinc-500"}`}>
                    {done ? <Check size={12} weight="bold" /> : <Icon size={12} />}
                  </span>
      ...
      <div className="p-8 min-h-[500px]">
        {step === 1 && <StepUpload photo={photo} setPhoto={setPhoto} onNext={() => goto(2)} />}
        {step === 2 && <StepSize size={size} setSize={setSize} />}
        {step === 3 && <StepCrop src={photo?.src} size={size} croppedSrc={croppedSrc} setCroppedSrc={setCroppedSrc} />}
        {step === 4 && <StepBackground src={finalPhotoSrc} bgWhite={bgWhite} setBgWhite={setBgWhite} setCroppedSrc={setCroppedSrc} />}
        {step === 5 && <StepDress />}
        {step === 6 && (
          <StepLayout
        ...
        {step === 7 && (
          <StepBilling
            ...
            copies={copies} rate={rate} setRate={setRate} size={size} paper={paper}
            refreshCustomers={() => api.get("/customers").then(({ data }) => setCustomers(data))}
          />
        )}
        {step === 8 && (
          <StepPrint
            tiles={tiles} paper={paper} size={size} copies={copies}
      ...
      <div className="border-t border-zinc-200 bg-white px-8 py-4 flex items-center justify-between sticky bottom-0">
        ...
        <div className="text-xs font-mono text-zinc-500">Step {step} of 8</div>
        {step < 8 ? (
          <button
            onClick={() => goto(step + 1)}
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      >
        ...
        <div className="mt-3 font-heading text-lg">Drop a photo here or click to upload</div>
        <div className="text-sm text-zinc-500 mt-1">JPG / PNG · Recommended front-facing portrait</div>
        <input
      ...
      {photo && (
        <div className="mt-6 flex items-center gap-4 bg-white border border-zinc-200 rounded-md p-4" data-testid="upload-preview">
          <img src={photo.src} alt="preview" className="w-24 h-32 object-cover rounded" />
          <div className="flex-1">
            <div className="font-medium">{photo.name}</div>
            <div className="text-xs text-zinc-500 mt-1">Ready to proceed. You can crop & align in step 3.</div>
          </div>
          <button onClick={onNext} className="h-10 px-4 bg-[#0052FF] text-white rounded-md text-sm">Continue</button>
        </div>
      )}
    </div>
  );
...
function StepSize({ size, setSize }) {
  return (
    <div className="space-y-4 max-w-5xl">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {PASSPORT_SIZES.map((s) => {
          ...
          return (
            ...
            >
              <div className="label-uppercase">{active ? "Selected" : s.custom ? "Custom" : "Passport"}</div>
              ...
              <div className="mt-3 flex items-end gap-2">
                <div
                  ...
                  style={{ width: Math.min(s.w, 60) * 1.4, height: Math.min(s.h, 80) * 1.4 }}
                />
                <div className="text-[10px] font-mono text-zinc-500 pb-1">{s.w}×{s.h} mm</div>
              </div>
            </button>
          );
        })}
      ...
      {/* Custom size editor */}
      {size.custom && (
        <div className="bg-white border border-[#0052FF]/40 rounded-md p-5" data-testid="custom-size-panel">
          <div className="flex items-center justify-between mb-3">
            <div>
              <div className="label-uppercase">Custom size</div>
              <div className="font-heading text-lg mt-1">Enter your exact dimensions</div>
              <div className="text-xs text-zinc-500 mt-0.5">
                Free-form crop with all edge & corner handles will be enabled in Step 3.
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div>
                ...
                <input
                  ...
                  value={size.w}
                  onChange={(e) => setSize({ ...size, w: Math.max(10, Math.min(300, Number(e.target.value) || 10)) })}
              ...
              <div>
                ...
                <input
                  ...
                  type="number" min="10" max="300" step="1"
                  value={size.h}
                  onChange={(e) => setSize({ ...size, h: Math.max(10, Math.min(300, Number(e.target.value) || 10)) })}
                  className="w-24 h-10 px-3 border border-zinc-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#0052FF]"
                />
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 pt-3 border-t border-zinc-200">
            ...
            {[[30, 40], [40, 50], [45, 60], [50, 70], [60, 90]].map(([w, h]) => (
              <button
                key={`${w}x${h}`}
...
function StepCrop({ src, size, croppedSrc, setCroppedSrc }) {
  ...
  useEffect(() => { setOutW(size.w); setOutH(size.h); }, [size.id, size.w, size.h]);
  ...
  if (!src) return <div className="text-zinc-500">Upload a photo in Step 1 first.</div>;
      ...
      <div className="bg-white border border-zinc-200 rounded-md overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2 border-b border-zinc-200">
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 text-xs text-zinc-600 cursor-pointer" data-testid="free-aspect-toggle">
              <input
                type="checkbox"
                checked={freeAspect}
              ...
              />
              <span className="uppercase tracking-[0.15em]">Free aspect (drag any edge)</span>
            </label>
        ...
        <div className="relative bg-zinc-900 crop-photoshop" style={{ height: 480 }} data-testid="crop-workspace">
          <Cropper
            image={src}
            crop={crop}
            zoom={zoom}
            ...
            style={{
              ...
              cropAreaStyle: { border: "2px solid #0052FF", color: "rgba(0,0,0,0.55)" },
            }}
          />
          {!freeAspect && (
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
              <div className="w-24 h-32 rounded-full border border-white/30" style={{ marginTop: -20 }} />
            </div>
          )}
        </div>
        <div className="p-4 border-t border-zinc-200 grid grid-cols-1 md:grid-cols-3 gap-4">
          <RangeRow label="Zoom" value={zoom} min="1" max="4" step="0.01" onChange={setZoom} testid="crop-zoom" />
          <RangeRow label="Rotate" value={rotation} min="-180" max="180" step="1" onChange={setRotation} testid="crop-rotate" />
          <div className="flex items-end gap-2">
            <button onClick={reset} className="h-9 px-3 border border-zinc-300 rounded-md text-xs uppercase tracking-[0.15em] flex-1">Reset</button>
            <button
        ...
        {freeAspect && (
          <div>
            ...
            <div className="flex items-center gap-2">
              <input
                type="number" min="10" max="300" value={outW}
                onChange={(e) => setOutW(Math.max(10, Number(e.target.value) || 10))}
                className="w-full h-9 px-2 border border-zinc-300 rounded-md text-sm"
                data-testid="crop-out-w"
              />
              <span className="text-zinc-400">×</span>
              <input
                type="number" min="10" max="300" value={outH}
                onChange={(e) => setOutH(Math.max(10, Number(e.target.value) || 10))}
                className="w-full h-9 px-2 border border-zinc-300 rounded-md text-sm"
                data-testid="crop-out-h"
              />
            </div>
            <div className="text-[10px] text-zinc-500 mt-1">Free-form mode. Output will be scaled to these mm dimensions.</div>
          </div>
        )}

        <div>
          <div className="label-uppercase mb-2">Preview {freeAspect ? `${outW}×${outH}mm` : `${size.w}×${size.h}mm`}</div>
...
function RangeRow({ label, value, min, max, step, onChange, testid }) {
  return (
    <div>
      <div className="flex justify-between text-xs mb-1"><span className="label-uppercase">{label}</span><span className="font-mono">{Number(value).toFixed(2)}</span></div>
      <input data-testid={testid} type="range" min={min} max={max} step={step} value={value} onChange={(e) => onChange(Number(e.target.value))} className="w-full" />
    </div>
  );
}

/* ---------------- Step 4: Background ---------------- */
function StepBackground({ src, bgWhite, setBgWhite, setCroppedSrc }) {
  const [busy, setBusy] = useState("");
  const [bgColor, setBgColor] = useState("#ffffff");

  if (!src) return <div className="text-zinc-500">Complete previous steps first.</div>;
  ...
  const aiFaceEnhance = async () => {
    setBusy("face");
    try {
      const { data } = await api.post("/ai/face-enhance", { image: src, fidelity: 0.7 });
      const enh = await loadImg(data.image_url);
      const c = document.createElement("canvas"); c.width = enh.width; c.height = enh.height;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-4xl">
      <div className="bg-white border border-zinc-200 rounded-md p-5">
        <img src={src} alt="preview" className="w-full max-h-[420px] object-contain bg-zinc-50 rounded" />
      </div>
      <div className="bg-white border border-zinc-200 rounded-md p-5 space-y-4">
        <div className="label-uppercase">Enhancement</div>

        <div className="space-y-2">
          <button onClick={overlayWhite} disabled={!!busy} data-testid="bg-white-btn"
            className="w-full h-11 border border-zinc-300 rounded-md text-sm text-left px-4 hover:border-[#0052FF] disabled:opacity-50">
            Lighten background (studio white)
          </button>

          <div className="border border-zinc-200 rounded-md p-3 space-y-2">
            <button onClick={aiCutout} disabled={!!busy} data-testid="ai-cutout-btn"
              ...
              {busy === "cutout" ? <span className="text-[10px]">Processing…</span> : <span className="text-[10px] uppercase tracking-[0.2em]">fal.ai</span>}
            </button>
            <div className="flex items-center gap-2 text-xs">
              <span className="label-uppercase">BG color</span>
              <input type="color" value={bgColor} onChange={(e) => setBgColor(e.target.value)} data-testid="bg-color"
          ...
          <button onClick={aiFaceEnhance} disabled={!!busy} data-testid="ai-face-btn"
            className="w-full h-11 border border-zinc-300 rounded-md text-sm text-left px-4 hover:border-[#0052FF] flex items-center justify-between disabled:opacity-60">
            <span>Face Enhance / Upscale</span>
            <span className="text-[10px] uppercase tracking-[0.2em] text-zinc-500">
              {busy === "face" ? "Processing…" : "fal.ai"}
            </span>
          </button>
        </div>

        {bgWhite && <div className="text-xs text-green-600 uppercase tracking-[0.2em]">✓ Background enhanced</div>}
      </div>
    </div>
  );
}
...
/* ---------------- Step 6: Layout Builder ---------------- */
function StepLayout({ paper, setPaper, size, copies, setCopies, tiles, setTiles, autoFill, finalPhotoSrc }) {
  ...
  const duplicateTile = (t) => setTiles([...tiles, { ...t, id: crypto.randomUUID(), x: t.x + 10, y: t.y + 10 }]);
  ...
  const onMouseDown = (e, t) => {
    ...
    const move = (ev) => {
      ...
      setTiles((prev) => prev.map((p) => p.id === t.id ? {
        ...p,
  ...
  const onResize = (e, t) => {
    ...
    const move = (ev) => {
      ...
      setTiles((prev) => prev.map(p => p.id === t.id ? { ...p, w: newW, h: newH } : p));
    };
    const up = () => { window.removeEventListener("mousemove", move); window.removeEventListener("mouseup", up); };
    window.addEventListener("mousemove", move); window.addEventListener("mouseup", up);
  };
          ...
          {tiles.map((t) => (
            <div
              ...
              style={{
                ...
                border: (t.borderWidth || 0) > 0 ? `${t.borderWidth}px solid ${t.borderColor || "#000"}` : undefined,
                borderRadius: t.borderRadius || 0,
              }}
            >
              <img src={t.src} alt="" draggable={false} style={{ borderRadius: Math.max(0, (t.borderRadius || 0) - (t.borderWidth || 0)) }} />
              {selected === t.id && (
                <>
                  <div className="resize-handle" onMouseDown={(e) => onResize(e, t)} />
                  <button onClick={(e) => { e.stopPropagation(); duplicateTile(t); }}
                    className="absolute top-1 right-8 w-6 h-6 bg-white/95 rounded shadow flex items-center justify-center text-zinc-700">
                    <Copy size={12} />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); removeTile(t.id); }}
      ...
      <aside className="bg-white border border-zinc-200 rounded-md p-4 space-y-4 h-fit">
        <div>
          ...
          <div className="mt-2 space-y-2">
            ...
            <StatRow label="Copies requested" value={copies} />
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-zinc-200">
          ...
          {(() => {
            const bw = selectedTile ? selectedTile.borderWidth || 0 : (tiles[0]?.borderWidth || 0);
            const bc = selectedTile ? selectedTile.borderColor || "#000000" : (tiles[0]?.borderColor || "#000000");
            const br = selectedTile ? selectedTile.borderRadius || 0 : (tiles[0]?.borderRadius || 0);
            const setW = (v) => selectedTile ? updateTile(selectedTile.id, { borderWidth: v }) : applyBorderToAll({ borderWidth: v });
            const setC = (v) => selectedTile ? updateTile(selectedTile.id, { borderColor: v }) : applyBorderToAll({ borderColor: v });
            const setR = (v) => selectedTile ? updateTile(selectedTile.id, { borderRadius: v }) : applyBorderToAll({ borderRadius: v });
              ...
              <div className="mt-3 space-y-3">
                <div>
                  ...
                  <input
                    ...
                    type="range" min="0" max="20" step="1" value={bw}
                ...
                <div>
                  ...
                  <input
                    ...
                    type="range" min="0" max="60" step="1" value={br}
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <div className="font-mono text-[11px] text-zinc-500">{bc}</div>
                </div>
                  ...
                  <div className="grid grid-cols-4 gap-1.5">
                    {[
                      ...
                      <button
                        key={p.label}
                        onClick={() => {
                          const patch = { borderWidth: p.w, borderColor: p.c, borderRadius: p.r };
                          selectedTile ? updateTile(selectedTile.id, patch) : applyBorderToAll(patch);
                        }}
                ...
                {!selectedTile && (
                  <div className="text-[10px] text-zinc-500 leading-relaxed">
                    Tip: click a photo on the canvas to edit just that photo&apos;s border.
                  </div>
                )}
              </div>
            );
        ...
        <div className="pt-3 border-t border-zinc-200 text-xs text-zinc-500 leading-relaxed">
          Drag tiles to reposition · corner handle to resize · click a tile to duplicate / delete.
        </div>
      </aside>
    </div>
  );
...
/* ---------------- Step 7: Billing ---------------- */
function StepBilling({ customers, customerId, setCustomerId, copies, rate, setRate, size, paper, refreshCustomers }) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-4xl">
      <div className="bg-white border border-zinc-200 rounded-md p-5 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowNew(!showNew)} className="text-[#0052FF] text-xs uppercase tracking-[0.2em]">
            {showNew ? "Choose existing" : "+ New customer"}
          </button>
        </div>
        {!showNew ? (
          <select value={customerId} onChange={(e) => setCustomerId(e.target.value)} data-testid="wizard-customer-select"
          ...
          <form onSubmit={createCustomer} className="space-y-2">
            <input required placeholder="Full name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
              className="w-full h-10 px-3 border border-zinc-300 rounded-md" data-testid="new-cust-name" />
            <input placeholder="Phone" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}
              className="w-full h-10 px-3 border border-zinc-300 rounded-md" />
            <button type="submit" className="h-10 px-4 bg-[#0052FF] text-white rounded-md text-sm">Save customer</button>
          </form>
        )}
      </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-md p-5 space-y-3">
        ...
        <div className="text-sm">{copies} × {size.label}</div>
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <input type="number" min="0" value={rate} onChange={(e) => setRate(Number(e.target.value))}
            className="w-28 h-9 px-2 border border-zinc-300 rounded-md text-right" data-testid="wizard-rate" />
        </div>
        <div className="mt-2 space-y-1 text-sm">
          <div className="flex justify-between"><span>Subtotal</span><span className="font-mono">{inr(subtotal)}</span></div>
          <div className="flex justify-between"><span>GST 18%</span><span className="font-mono">{inr(gstAmt)}</span></div>
          <div className="flex justify-between font-heading text-lg font-semibold border-t border-zinc-200 pt-2"><span>Total</span><span data-testid="wizard-total">{inr(total)}</span></div>
        </div>
...
/* ---------------- Step 8: Print & Finalize ---------------- */
function StepPrint({ tiles, paper, size, copies, customerId, customers, afterFinalize }) {
  const [busy, setBusy] = useState(false);
  ...
  const exportPdf = async () => {
    ...
    const canvas = await html2canvas(paperRef.current, { scale: 3, backgroundColor: "#ffffff" });
    const img = canvas.toDataURL("image/jpeg", 0.95);
    const isLandscape = paper.w > paper.h;
    const pdf = new jsPDF({ orientation: isLandscape ? "l" : "p", unit: "mm", format: paper.id === "a4" ? "a4" : [paper.w, paper.h] });
  ...
  const finalize = async () => {
    if (!customerId) { toast.error("Select a customer in step 7"); return; }
    setBusy(true);
    try {
      // Order
      ...
      const { data: inv } = await api.post("/invoices", {
        customer_id: customerId,
        items: [{ description: `${copies} × ${size.label} on ${paper.label}`, quantity: 1, rate: 150, gst_rate: 18 }],
        payment_method: "cash",
        status: "paid",
        notes: `Passport photo job (${copies} copies)`,
      });
      setInvoice(inv);
        ...
        >
          {tiles.map(t => (
            <div key={t.id} style={{
              ...
              transform: `rotate(${t.rotation}deg)`, overflow: "hidden",
              border: (t.borderWidth || 0) > 0 ? `${t.borderWidth}px solid ${t.borderColor || "#000"}` : undefined,
              borderRadius: t.borderRadius || 0,
              ...
              <img src={t.src} alt="" style={{
                ...
                borderRadius: Math.max(0, (t.borderRadius || 0) - (t.borderWidth || 0)),
              }} />
            </div>
          ))}
        </div>
      ...
      <div className="space-y-3">
        <div className="bg-white border border-zinc-200 rounded-md p-5">
          ...
          <div className="mt-2 text-sm space-y-1">
            <div><span className="text-zinc-500">Customer:</span> <span className="font-medium">{cust?.name || "—"}</span></div>
            <div><span className="text-zinc-500">Size:</span> {size.label}</div>
            <div><span className="text-zinc-500">Paper:</span> {paper.label}</div>
            <div><span className="text-zinc-500">Copies:</span> {copies}</div>
            <div><span className="text-zinc-500">Tiles placed:</span> {tiles.length}</div>
          </div>
        </div>

        <button onClick={exportPdf} data-testid="export-pdf" className="w-full h-11 border border-zinc-300 rounded-md text-sm flex items-center justify-center gap-2"><DownloadSimple size={16} /> Export PDF</button>
        <button onClick={exportPng} className="w-full h-11 border border-zinc-300 rounded-md text-sm flex items-center justify-center gap-2"><DownloadSimple size={16} /> Export PNG</button>
        <button onClick={() => window.print()} className="w-full h-11 border border-zinc-300 rounded-md text-sm flex items-center justify-center gap-2"><Printer size={16} /> Print</button>

        {!invoice ? (
          <button onClick={finalize} disabled={busy} data-testid="finalize-btn" className="w-full h-12 bg-[#0052FF] hover:bg-[#0040CC] text-white rounded-md font-medium">
          ...
          <div className="bg-green-50 border border-green-200 rounded-md p-4 text-sm">
            ...
            <div className="font-mono text-xs text-green-700 mt-1">{invoice.invoice_no}</div>
            <button onClick={afterFinalize} className="mt-3 w-full h-10 bg-zinc-900 text-white rounded-md text-sm">Back to dashboard</button>
          </div>
        )}
      </div>
    </div>
