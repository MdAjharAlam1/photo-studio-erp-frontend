...
//   const [autoCenter, setAutoCenter] = useState(true);
//   const [keepTogether, setKeepTogether] = useState(true);
//   // 0 = auto-fit using the chosen passport size's real mm dimensions (old behavior).
//   // >0 = user picks how many go across, and photo size/row count adjust automatically.
...
//   // Grid geometry for the chosen passport size + paper + gaps/margin.
//   // If photosPerRow is set, the tile size is auto-derived to fit exactly that
//   // many columns (keeping the passport size's aspect ratio), and the row count
//   // is then auto-calculated from how much vertical space is left — the person
//   // only picks the column count, everything else adjusts automatically.
//   const grid = useMemo(() => {
//     const pW = paper.w * MM_PX;
//     const pH = paper.h * MM_PX;
...
//     // if (photosPerRow > 0) {
//     //   const aspect = size.h / size.w; // preserve passport size's proportions
//     //   const cols = photosPerRow;
//     //   const w = Math.max(10, (availableW - (cols - 1) * hGap) / cols);
//     //   const h = w * aspect;
//     //   const rows = Math.max(1, Math.floor((availableH + vGap) / (h + vGap)));
//     //   return { pW, pH, w, h, cols, rows, capacity: cols * rows };
//     // }
...
//   const rawW = (availableW - (cols - 1) * hGap) / cols;
//   const w = Math.max(10, rawW);
//   const h = w * aspect;

//   const fitsWidth = rawW >= 10;          // cols actually fit at a usable size
//   const fitsHeight = h <= availableH;    // a single row isn't taller than the page
...
//     pW, pH, w, h, cols, rows,
//     capacity: cols * rows,
//     overflowsWidth: !fitsWidth,
//     overflowsHeight: !fitsHeight,
//   };
// }

//     // Auto-fit mode (photosPerRow === 0): use the real mm size of the passport photo.
//     const w = size.w * MM_PX;
//     const h = size.h * MM_PX;
//     const cols = Math.max(1, Math.floor((availableW + hGap) / (w + hGap)));
//     const rows = Math.max(1, Math.floor((availableH + vGap) / (h + vGap)));
//     return { pW, pH, w, h, cols, rows, capacity: cols * rows };
//   }, [paper, size, hGap, vGap, margin, photosPerRow]);
...
//   const totalCopies = items.reduce((s, i) => s + (i.copies || 0), 0);
//   // GST removed entirely — this printing business doesn't collect GST from customers,
//   // so billing is now a flat subtotal (copies × rate) with no tax line at all.
//   const billingSubtotal = items.reduce((s, i) => s + (i.copies || 0) * (i.rate || 0), 0);
//   const billingTotal = billingSubtotal;
//   const inr = (n) => "₹" + Number(n || 0).toLocaleString("en-IN");
...
//             {/* Photos per row — set the column count and everything else (photo
//                 size, row count) auto-adjusts to fit the page. */}
//             <div>
...
//                 value={photosPerRow}
//                 onChange={(e) =>
//                   setPhotosPerRow(Math.max(0, Math.floor(Number(e.target.value)) || 0))
//                 }
//                 className="w-full h-9 border border-zinc-300 rounded-md px-2 text-sm"
//                 data-testid="mpp-photos-per-row"
//               />
//                 {(grid.overflowsWidth || grid.overflowsHeight) && (
//                   <div className="text-[10px] text-red-600 mt-1">
//                     {photosPerRow} per row doesn't fit this paper size — reduce the count or gaps.
//                   </div>
//                 )}
//               <div className="text-[10px] font-mono text-zinc-500 mt-1">
//                 {photosPerRow > 0
//                   ? `Photo size auto-adjusted to fit ${photosPerRow} across — ${grid.rows} row(s) fit automatically`
//                   : "0 = auto-fit using the passport size's real mm dimensions"}
//               </div>
//             </div>
...
//               <div>
//                   <div className="flex justify-between text-xs mb-1">
//                     <span className="label-uppercase">H. Gap</span>
//                     <span className="font-mono">{hGap}px</span>
//                   </div>
//                     <input
//                       type="range"
//                       min={0}
...
//           {/* GST removed — flat subtotal = total, since this business doesn't
//               collect GST from these customers. */}
//           <div className="border-t border-zinc-200 pt-3 space-y-1 text-sm max-w-xs ml-auto">
//             <div className="flex justify-between">
//               <span>Subtotal</span>
//               <span className="font-mono">{inr(billingSubtotal)}</span>
...
export default function MultiPhotoPassport() {
  ...
  // Grid geometry for the chosen passport size + paper + gaps/margin.
  // If photosPerRow is set (>0), the tile size is auto-derived to fit exactly
  // that many columns (keeping the passport size's aspect ratio), and the row
  // count is auto-calculated from remaining vertical space. If photosPerRow
  // is 0, auto-fit mode uses the passport size's real mm dimensions.
  const grid = useMemo(() => {
    const pW = paper.w * MM_PX;
    const pH = paper.h * MM_PX;
    const availableW = pW - 2 * margin;
    ...
    if (photosPerRow > 0) {
      ...
      const fitsWidth = rawW >= 10;       // requested column count fits at a usable size
      const fitsHeight = h <= availableH; // a single row isn't taller than the page

      const rows = fitsHeight
        ? Math.max(1, Math.floor((availableH + vGap) / (h + vGap)))
        : 0; // 0 rows signals "doesn't fit" instead of forcing an overflowing row
      ...
      <div className="p-8 grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6">
        ...
        <aside className="space-y-5">
          <div className="bg-white border border-zinc-200 rounded-md p-4 space-y-4">
            ...
            {/* H. Gap / V. Gap — each bound to its own state, each correctly labeled */}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <div className="flex justify-between text-xs mb-1">
                  <span className="label-uppercase">H. Gap</span>
              ...
              <div>
                <div className="flex justify-between text-xs mb-1">
                  <span className="label-uppercase">V. Gap</span>
                  <span className="font-mono">{vGap}px</span>
                </div>
                <input
                  type="range"
                  min={0}
            ...
            {/* Page Margin — its own slider, bound to margin state */}
            <div>
              <div className="flex justify-between text-xs mb-1">
                <span className="label-uppercase">Page Margin</span>
